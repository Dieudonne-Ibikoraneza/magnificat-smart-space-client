    ...
    <div className="mx-auto max-w-300">
      ...
      <section className="rounded-3xl bg-white p-6 sm:p-8 lg:px-10 lg:py-9">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <Button type="button" onClick={saveChanges} className="h-11 gap-2 px-6 text-sm font-bold">
            <Save className="size-[18px]" /> Save Changes
          </Button>
          <Button type="button" variant="outline" onClick={cancelChanges} className="h-11 gap-2 px-6 text-sm font-bold text-ink hover:bg-slate-50 hover:text-ink">
      ...
      <section className="mt-5 rounded-3xl bg-white p-6 sm:p-8 lg:px-10 lg:py-9">
        ...
        <div className="mt-5 rounded-lg border border-red-200 p-5 sm:p-6">
          ...
          <p className="mt-3 max-w-5xl text-sm leading-5 text-muted">
            Deleting your account is permanent. All your data, including order history and saved projects, will be removed from our systems and cannot be recovered.
          </p>
          {!accountDeleted ? (
            <Button type="button" variant="outline" onClick={() => setDeleteDialogOpen(true)} className="mt-5 h-12 gap-3 border-red-500 px-5 text-base font-bold text-red-500 hover:border-red-600 hover:bg-red-50 hover:text-red-600">
      ...
      {deleteDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="delete-dialog-title">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 id="delete-dialog-title" className="text-xl font-bold text-ink">Delete account?</h2>
            ...
            <label className="mt-6 block text-sm font-semibold text-ink">
              ...
              <Input autoFocus value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} placeholder="Delete" className="mt-2 h-12" />
            </label>
