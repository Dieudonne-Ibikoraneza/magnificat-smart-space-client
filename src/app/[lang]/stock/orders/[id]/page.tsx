...
const OrderDetailPage = ({ params }: OrderDetailPageProps) => {
  ...
  return (
    <>
      <StockDetailHeader
        breadcrumbs={[
          { label: t("sales.orderDetail.crumbOverview"), href: "/stock/overview" },
          { label: t("sales.orderDetail.crumbOrders"), href: "/stock/orders" },
                ...
                <ul className="divide-y divide-[#E8E8E8]">
                  {items.map((item) => (
                    <li key={item.id} className="px-5 py-4 font-data">
                      <Link href={"/stock/inventory/" + item.productId} className="flex items-center gap-3 font-semibold text-ink uppercase hover:underline">
                        {item.product?.image && (
                    ...
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell className="font-medium text-ink uppercase">
                          <Link href={"/stock/inventory/" + item.productId} className="flex items-center gap-3 hover:underline">
                            {item.product?.image && (
