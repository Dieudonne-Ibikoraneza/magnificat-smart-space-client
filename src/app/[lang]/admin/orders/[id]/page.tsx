...
const OrderDetailPage = ({ params }: OrderDetailPageProps) => {
  ...
  useEffect(() => {
    if (autoOpenDelivery) router.replace(`/admin/orders/${id}`);
                ...
                <ul className="divide-y divide-[#E8E8E8]">
                  {items.map((item) => (
                    <li key={item.id} className="px-5 py-4 font-data">
                      <Link href={"/admin/inventory/" + item.productId} className="flex items-center gap-3 font-semibold text-ink uppercase hover:underline">
                        {item.product?.image && (
                    ...
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell className="font-medium text-ink uppercase">
                          <Link href={"/admin/inventory/" + item.productId} className="flex items-center gap-3 hover:underline">
                            {item.product?.image && (
                ...
                <dl className="mt-5 space-y-5 text-sm">
                  <div>
                    ...
                    <dd className="mt-2 flex items-center gap-3">
                      ...
                      <Link href={"/admin/customers/" + order.customer.id} className="font-medium text-ink underline decoration-primary decoration-2 underline-offset-4 hover:opacity-70">
