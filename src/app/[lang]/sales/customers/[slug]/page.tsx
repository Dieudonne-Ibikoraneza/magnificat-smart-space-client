...
import { DashboardDetailHeader as SalesDetailHeader } from "@/components/dashboard-page-headers";
...
const CustomerDetailPage = ({ params }: CustomerDetailPageProps) => {
  ...
  // `ordersPage` is deliberately not in `deps` below — paging the orders
  // list re-runs the same query (via `reload()`, reading the latest
  // `ordersPage` through the fetcher closure) instead of a "genuinely
  // different query", so the whole profile doesn't blank out while only the
  // orders table refreshes. See `useApi`'s doc comment for that distinction.
    ...
    <>
      <SalesDetailHeader
        ...
        actions={
          <Button
            nativeButton={false}
            render={<Link href={`/sales/orders/new?customer=${customer.id}`} />}
            ...
            {orders.length === 0 ? (
              ...
              <>
                ...
                <ul className="divide-y divide-[#E5E7EB] md:hidden">
                  {orders.map((order) => (
                    <li key={order.id}>
                      <Link href={"/sales/orders/" + order.id} className="flex items-start justify-between gap-3 px-5 py-4 font-data transition-colors hover:bg-secondary/50">
                        <div>
