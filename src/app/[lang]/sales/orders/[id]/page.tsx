...
const OrderDetailPage = ({ params }: OrderDetailPageProps) => {
  ...
  useEffect(() => {
    if (autoOpenDelivery) router.replace(`/sales/orders/${id}`);
    ...
    if (error.toLowerCase().includes("not found") || error.toLowerCase().includes("access")) {
      return (
        <div className="mx-auto max-w-md py-24 text-center">
          ...
          <Button nativeButton={false} render={<Link href="/sales/orders" />} className="mt-6 h-11 gap-2 px-5">
  ...
  return (
    <>
      <SalesDetailHeader
        breadcrumbs={[
          { label: t("sales.orderDetail.crumbOverview"), href: "/sales/overview" },
          { label: t("sales.orderDetail.crumbOrders"), href: "/sales/orders" },
                ...
                <ul className="divide-y divide-[#E8E8E8]">
                  {items.map((item) => (
                    <li key={item.id} className="px-5 py-4 font-data">
                      <Link href={"/sales/catalog/" + item.productId} className="flex items-center gap-3 font-semibold text-ink uppercase hover:underline">
                        {item.product?.image && (
                    ...
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell className="font-medium text-ink uppercase">
                          <Link href={"/sales/catalog/" + item.productId} className="flex items-center gap-3 hover:underline">
                            {item.product?.image && (
