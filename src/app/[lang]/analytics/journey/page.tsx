...
const actionTargetOf = (action: JourneyStageAction): { href: string; labelKey: string } | null => {
  switch (action.type) {
    ...
    case "QUOTE_NEGOTIATING": {
      ...
      return orderId ? { href: `/analytics/orders/${orderId}`, labelKey: "analytics.journey.viewOrder" } : null;
    }
    case "ORDER_NEGOTIATION": {
      const orderId = readString(action.detail, "orderId") ?? action.id;
      return { href: `/analytics/orders/${orderId}`, labelKey: "analytics.journey.viewOrder" };
    }
    case "ORDER_PLACED":
    case "ORDER_PURCHASED":
      return { href: `/analytics/orders/${action.id}`, labelKey: "analytics.journey.viewOrder" };
    case "TILE_VIEWED":
    case "TILE_APPLIED": {
      const productId = readString(action.detail, "productId");
      return productId ? { href: `/analytics/tiles/${productId}`, labelKey: "analytics.journey.viewTile" } : null;
    }
    default:
      return null;
  }
                          ...
                          <TableCell>
                            {!target ? (
                              profile ? (
                                ...
                                <Link
                                  href={`/analytics/customers/${profile.id}`}
...
const AnalyticsJourneyPageContent = () => {
  const { t } = useTranslation();
...
const AnalyticsJourneyPage = () => (
  <Suspense fallback={null}>
    <AnalyticsJourneyPageContent />
  </Suspense>
);

export default AnalyticsJourneyPage;
