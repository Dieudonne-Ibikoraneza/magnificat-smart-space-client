...
export const KpiCards = ({ items }: { items: KpiCardData[] }) => (
  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    {items.map(({ label, value, icon: Icon, trend, badge }) => {
      ...
      return (
        <article key={label} className="flex flex-col h-full rounded-2xl bg-card p-5 sm:p-6">
          <Icon className="size-5 stroke-2 text-ink" />
          <div className="mt-4 flex flex-1 flex-col justify-end">
            <p className="text-[11px] font-bold tracking-wide text-muted-foreground uppercase">
              {label}
            </p>
            <div className="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-1 font-black">
